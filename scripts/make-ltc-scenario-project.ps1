...
#
#   powershell -File scripts\make-ltc-scenario-project.ps1 -MediaDir D:\media
#   powershell -File scripts\make-ltc-scenario-project.ps1 -MediaDir D:\media -Out D:\media\ltc-scenario.tsp -Tracks 3 -SegmentSeconds 20
#
...
# a 5 s offset and keeps a 5 s gap after every track. The .tsp and the media must
# live in the same directory (the app resolves track paths relative to the project
# file and rejects paths outside of it).
#
...
if ([string]::IsNullOrWhiteSpace($Out)) {
    $Out = Join-Path $MediaDir 'ltc-scenario.tsp'
}
$Out = [System.IO.Path]::GetFullPath($Out)
$outDir = [System.IO.Path]::GetDirectoryName($Out)
if (-not [string]::Equals($outDir.TrimEnd('\'), $MediaDir.TrimEnd('\'), [System.StringComparison]::OrdinalIgnoreCase)) {
    throw 'Out must be placed directly under MediaDir (the app resolves tracks relative to the project file)'
}
