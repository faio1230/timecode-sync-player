...
| V8 | **基準を確定**（40 秒で落ち 5 回以下かつ連続なし）。**V8-D は差し戻し中**: MMCSS を既定で有効にすると落ちが 6/9/7 で基準を満たさず、環境変数で明示した場合（0/2/3）と食い違う。同一セッションで off/既定/high を交互に測らせて切り分け中 |
| ~~V7~~ | 消滅（mpv 除去による） |
...
1. **V8-D の差し戻し対応**（`docs/prompts/2026-09-15-V8D-rework.md`）→ V8 確定。
   **「既定で有効」が「環境変数で明示」より悪い**という食い違いの説明が要る。
   同一セッションで `off` / 既定 / `high` を**交互に**各 3 本測り、**既定と明示 high が一致するか**を見る。
   一致しなければ実装の欠陥。実装自体（失敗フック・無効化スイッチ・起動ログ・E2E）は良い。
   **60 分 run は完走したが、食い違いが解けるまで V6 の判定には使わない。生データは保全。**
