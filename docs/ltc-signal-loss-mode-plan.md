...
| 検出 | 最後の有効 LTC フレーム受信から **250ms**（既定）無受信で信号断と判定。同一TC連発など、TCが進まず有効フレームとして採用されない停滞も「有効フレームの無受信」に含む。AppSettings `LtcSignalLossTimeoutMs`（int、既定250、100〜5000にクランプ） |
| 発動条件 | **Sync ON かつ LTC 監視中**のみ。Single / Continue 両モード共通。GapFreezeHandler がアクティブ（ギャップ演出中）のときは介入しない。LTC STOP ボタン押下（手動停止）では発動しない（監視終了であり信号断ではない）。デバイス切断等、例外付きの監視異常停止は信号断として扱う |
| 停止動作 | **pause（最終フレーム表示のまま静止）**。黒画面にはしない（異常系で黒を出すと事故に見える） |
| トリガー方式 | **エッジトリガー**: 「受信中→信号断」への遷移後、停止条件が成立した時点で1回だけ pause する。断中にランスルーから停止へ切り替えた場合やSync ONにした場合も次のtickで再評価する。停止中にオペレーターが手動で Play した場合は手動が勝ち、ポリシーは再介入しない（次の「復帰→断」サイクルまで） |
...
| 永続化 | **AppSettings のみ**（GapBehavior と同じ流儀）。プロジェクトファイルには保存しない。起動時のsettings.json全読み込みを有効化したため、新項目に加えて既存の保存済み設定（Timeline表示／AutoOffset）も復元される。従来は保存のみで未読込だった潜在バグを併せて解消した |
...
- AppSettings に3項目追加（モード・タイムアウトms・復帰フレーム数）。
  後方互換: 既存 settings.json に無いキーは既定値。`LtcSignalLossTimeoutMs` と
  `LtcSignalResumeFrames` はUIから変更せず、ファイルを変更した場合はアプリ再起動が必要
- E2E起動時は環境変数 `TIMECODE_SYNC_PLAYER_SETTINGS_PATH` で一意なテスト用設定ファイルを
  指定し、実ユーザーのsettings.jsonを読み書きしない
- 信号断の経過時間はシステム時刻補正の影響を受けない単調時刻で判定する
