...
**M6 の傍証（2026-09-19、フェーズ 2 の受け入れ条件に追加）**: ダーティー LTC の M6 で、
**600ms の欠落（外挿上限 0.5 秒超）後も復帰後に値が化けない**ことを確認した。フェーズ 2 で
外挿の入力（配信 PTS + 世代）を変えても、この性質（欠落中は配信 PTS で頭打ちし、復帰時に
不連続に跳ばない）を壊さないこと。M6 ではサンプルレート不一致（44.1k/96k→48k）と ±100ppm の
速度差も実害なしで、LTC 速度推定を 0.4.5 でやらない判断の外部条件での裏づけになっている。
...
## フェーズ 2 の実機確認（2026-09-19 01:00、開発機、親）

**`TCS_SYNC_POSITION_FEEDBACK=on` で V3 を 1 本。**

| 項目 | D37-d（フェーズ 1） | **フェーズ 2 on** |
| --- | ---: | ---: |
| 定常 平均 | −27.8ms | −28.9ms |
| **定常 p95−p5** | 38.2ms | **35.0ms** |
| seek-a / b / c / back | 160 / 120 / 120 / 160 | 120 / 120 / 160 / 171 |
| black-sweep | 400ms | 392ms |
| freeze-sweep | 252ms | 120ms |

**すべて非回帰。収束は全フェーズ 1 秒以内。**
p95−p5 の差は量子化の床（LTC25 × 映像 24/29.97/60）を含む値なので誤差の範囲だが、
**悪化していないことが確認できた。**
...
| `evalBasis` | 件数 | 意味 |
| --- | ---: | --- |
| `pipeline` | 1,964 | 着地済み（評価位置 = クエリ値）。**定常では何も変わらない** |
| **`delivered`** | **2** | **着地未確認の区間。ここだけ配信 PTS 基準で評価している** |

**設計どおり。** V3 は短いシークしか含まないため該当区間が 2 件だが、
**長 GOP の素材ではこの区間が長くなり、そこで効く**
（フェーズ 1 の実測で、その区間の誤差が 393 → 547 → 751ms と育つことを確認済み）。

### 残り

- **長 GOP（L-1）での確認**。フェーズ 2 の効果が出るのはそちら
- 既定を `on` にするかの判断は、検証機の M3 で「着地時の不連続が消えるか」を見てから
