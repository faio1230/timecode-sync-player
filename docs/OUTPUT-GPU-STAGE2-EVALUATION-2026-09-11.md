...
対象: ブランチ `codex/output-engine-20260911-1247`（tip `bdfa34f`、基点 `3ac70d6`＋GStreamer ブランチ `5eb7e62` を merge）。親は独立の検証用 worktree `C:\Users\codea\Documents\timecode-sync-player-wt-verify-oe-20260911-1344`（detached、同 tip）でビルド・テスト・実機を行った。ネイティブ DLL（libmpv-2.dll、SpoutDX.dll）は session-refactor の `native/` から実行ディレクトリへコピーした。
...
## 追記: E1 `db99756`（→ main `4db2730`）の親評価（2026-09-13 01:58〜02:11 JST）

GStreamer E2E の素材と受信ツールを揃えたところ現れた `GStreamerBackend_SurvivesRepeatedTrackSwitches` の
不安定な判定への対応（テストのみ、製品コードは不変）。

1 回目の提出 `6f6158c` は**却下**した。`LoadFile path=` の総数判定（競合状態）は解消したが、
`Playlist track loaded index=` を**日毎ログ全体**に対する部分列で判定していたため、
ログが run をまたいで追記される以上、同じ日の 2 回目以降は過去 run の `0,1,2,3,2,1,0` を拾って
**切替を 1 度も行わないうちに真になる**。親のログに当該の並びが実際に残っていることを確認して差し戻した
（`docs/prompts/2026-09-13-E1-rework.md`）。

修正版 `db99756` は `runner.Process.StartTime` 以降のログ行だけを対象にし、期待列を初回ロードを除いた
`1,2,3,2,1,0` にした。E2E は `[CollectionDefinition("E2E", DisableParallelization = true)]` で直列化されており、
他テストのアプリが同時に同じログへ書くことはないので、時刻による絞り込みはこの前提の下で健全。

| 確認 | 結果 |
| --- | --- |
| **反証（期待列を `1,2,3,3,1,0` に改変）** | **失敗**。出力された実際列は `[0,1,2,3,2,1,0]` の 7 件で、日毎ログの 179 件ではなくこの run の分だけを見ていることの直接の証拠 |
| 対象テスト 3 回連続 | 成功（7s／7s／7s） |
| 全 E2E | **58 成功・0 失敗・4 スキップ**（残るスキップは環境変数で有効化する 4 件） |
| 非E2E | 1712 件成功 |

**判断: E1 合格。main へ rebase して ff 統合（`4db2730`）。**

検証の手順として、反証（わざと落として、落ちることを確かめる）は「3 回連続成功」より強い。
1 回目の提出は 3 回連続成功していたが常に真になる判定だった。以後、判定ロジックを変える修正では反証を必須にする。
