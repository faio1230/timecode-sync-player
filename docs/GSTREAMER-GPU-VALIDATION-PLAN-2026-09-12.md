...
- D35 の補足（検証機の解析）: R-1 の合否は「保持直前の行き過ぎが同期の許容 0.240 秒を超えたか」で決まる。超えた回は reapply 直後の decision が Seek になり着地（run b +0.000、run c +0.167）、超えない回（0.0167〜0.240 秒の帯）は速度補正だけで一時停止し失敗（run a +0.183、別 run 12.55→13.000）。**設計確定: 停止モードで保持に入ったら許容判定を経由せず保持値へ明示的に 1 回着地（1 フレーム以内なら省略）。許容 0.240 は下げない**。候補 5 の R 系基準値: R-1 +0.183 / +0.000 / +0.167、R-3 advance −8.917 / −9.250 / −9.267、R-4 resync −0.137 / −0.110 / −0.237。テスト側: R-2 に hold ジャーナル追加、R-3 の記録項目名の見直し（検証機がパッチ）
- 検証機のテスト側パッチ（R-2 に hold-pause、R-3 の項目名 holdTarget/observeSeconds/positionBefore/positionAfter/advancedSeconds）を統合。候補 5 での R 系（M2,M1,M4）: R-1 +0.150、**R-2 +0.450**（これまで判定外で見えず）、R-3 advancedSeconds 2.717、R-4 −0.130。検証機の集計で「C / G / S」と番号落ちしていた表記は C-2 / G-5 / S-5 等の誤記（trx 由来の表は正しい）
- **統合後 main（D34 込み）の実機確認（除去担当、2026-09-18 05:55、親が trx を確認）**: シナリオ 22/22、LTC ループ 14/14、S-3（MediaOut 8 < 尺 12）1/1（`clip boundary hold ltc=28.000 playback=8.044`）。shim ログ `0x0@` 0 件、成功ロード 173、`caps-missing` 6（h265-gpu/vp9-gpu/av1-gpu の不一致）。S-3 の判定許容 ±2 フレームを統合（agent-b `951d1ef`）。**候補 6 は D35 の統合後**

### D35 の実装（同期担当 agent-a `031d02f` / `88c36d1`、2026-09-18 05:56、実機中）

- 項目 1・2（`031d02f`）: 停止モードの一時停止時に損失理由・保持値の到着順に依らず `ReapplyHeldValueOnPause`（無音で値未確定なら最初の Duplicate で発行）。着地は tolerance 0.240 を経由せず、|位置 − 着地先| が 1 映像フレーム以内のときだけ省略。着地済み保持値は `_heldLossLandingSeconds` でラッチ（D31-b 維持）。pause 直前に Smooth の残り倍率を rate.instant で 1.0 へ（拒否時は次フレームで再試行）
- 項目 3（`88c36d1`）: ロード解除を期限付きに（強制解除、親の指示で 2 秒 → 5 秒。実素材のプロファイル試行 2.2〜2.5 秒を下回らないため）、解除の鮮度 1.5 秒で古い解除は破棄。Reverse は特別扱いせず
- 単体 +9（StopModeHeldLandingTests 5: 帯 0.18 秒で明示着地 / 0.55 秒 / 無音損失→後の Duplicate / 1 フレーム以内は省略 / 倍率復帰。TimecodeSyncServiceTests 4）。既存 2 件を新設計に合わせて調整。非E2E 1885/0
- 実機（合図済み）: R-1〜R-4 を生成素材と 4K 3 本で各 3 サイクル、S-2、C-1/C-2。hold-pause の超過と landing seek の発行を報告

### D35 の実機結果と統合（同期担当の実機、2026-09-18 06:09、親が trx を確認）
...
### D36（起票のみ。修正は 0.4.3 公開後）: UI 更新スケジューラで再スケジュール要求が失われうる（2026-09-18 08:02）

CI で断続的に落ちていたテスト（`RenderSessionTests.Callback_DispatchesFrameUpdateOnUiAndCoalescesRequests`、
直近 40 run で 7 件）の原因を追ったところ、**製品側にも競合が残っている**ことが分かった。

- ネイティブ側の `RequestDispatch`（合流を数えてから再スケジュール要求を立てる）と、前回 drain の
  `CompleteDispatch`（要求を読んで消す）が交差すると、要求が 1 回分失われる
- **絵は止まらない**（合成はリングから最新フレームを直接取る。この経路は UI 状態の更新のみ）
- 失われても次のフレーム通知で回復する（60fps で約 17ms、24fps で約 42ms）
- フレーム通知が止まる瞬間（一時停止・EOS 直後）でも、**`OnTick` が 100ms ごとにギャップ・テストカード・配置・位置を無条件に再送する**ため、残る遅れは最悪 100ms 程度
- 現行のログ・トレースからは、この事象が起きた回数を後から数えられない（数えるならスケジューラ側に記録の追加が要る）

**判定: 実害は限定的。0.4.3 の公開後に直す。** いま製品を変えると、検証機で通した候補 6（3 通し 36/36）が無効になり、
もう一度 45 分の通しが要る。CI 側はテストを時間非依存に直して解消済み（`ad32e07`、同一テスト 20/20）。

### L-1（60 秒連続追従）の実素材結果と D37 の起票（2026-09-18 08:40〜09:00、検証機と開発機）

新テスト L-1 を検証機の実素材で回し、**既存 36 本では出なかった欠陥を 1 件見つけた**。

#### 0.4.3（測り方の修正後、実素材 6 本）

| 素材 | 経路 | 判定 |
| --- | --- | --- |
| ProRes 4K60 | CPU デコード | 合格（最大誤差 0.107、49.5 秒に丸め） |
| AV1 4K24 ×2 | CPU デコード | 合格（0.097 / 0.093） |
| H.264 1080p60 | GPU | 合格（0.223） |
| VP9 1080p60 | GPU | 着地直後の過渡（窓 0・1）のみで失敗。**定常は健全**（位置停止 0、平均更新 117.8/120） |
| **VP9 4K60** | GPU | **失敗（D37）** |

**心配していた CPU デコードの 4K（ProRes・AV1）は詰まり 0 だった。**

#### 退行ではないことの確定

公開版 0.4.2（GitHub の資産、SHA-256 照合済み）で同じ素材を回すと、**L-1 の追従に入る前に失敗する**
（一時停止シークを出しても新位置のフレームが来ず `position=0` のまま。shim は `paused-seek: pump deadline`）。
**0.4.3 は公開版より明確に良い状態。** L-1 の失敗は今回の修正が作ったものではなく、新しいテストが初めて可視化した性質。

#### D37: 重い素材で補正シークが連鎖する

- 検証機（VP9 4K60、候補 6）: 60 秒の追従中に補正シーク **15 回**（平均 740ms、最長 2,022ms、保留の 2 秒解除 1 回）。
  同じ 60 秒で H.264 1080p60 は 6 回、AV1 4K24 は 2 回、VP9 1080p60 は 1 回
- 検証機（0.4.3、測り方の修正後）: `updates=18 / posAdvance=0.000 / ltcAdvance=1.920 / err=1.910`、`updates=0 / posAdvance=2.867 / err=3.710` など。
  **LTC の進みと位置の進みが乖離する**のが決め手（M4 の誤検出とはここが違う）
- 開発機の再現（生成 4K60、キーフレーム 10 秒）: 1 回のシークの着地は **0.46〜1.75 秒**で、目標がキーフレームを越えるまで比例して伸び、
  **越えた瞬間 184ms** に落ちる。その間 `compose.publish` は 60/s のまま、`gst.position` も単調前進で、
  **逼迫しているのは `gst.delivery`**（60/s → 9〜28/s、最悪 0/s）。位置が LTC に最大 3 秒遅れる
- 機序: accurate シークでキーフレームから復号し直す間に絵の供給が細る → ずれが増える → 許容 0.240 秒を超えて次のシーク → 連鎖。
  **保留中は Smooth が止まる**（`LtcSyncController.cs:709`）ので、いちばんずれている間に速度補正が使えない
- 直し方の候補（**未決。数字を見て決める**）: 保留の時間切れ（2 秒固定）を実測の着地時間に合わせる／シークの実測コストが高い素材では
  粗い判定のしきい値を上げて速度補正に寄せる／保留中も速度補正を許す

#### D38 候補は取り下げ（測定側の問題だった）

「位置が進まない窓」は、L-1 が位置と LTC を **UI の表示（UIA）から読んでおり**、
2 秒窓に入るサンプルが 40 個の想定に対して実際は 1〜17 個（中央値 5）しか無かったことが原因。
窓の進みを「窓内の最初と最後の差」で出していたため、サンプルが少ない窓で小さく出ていた。
検証機が `LtcAdvance` と `Samples` を足し、**起点を前の窓の最後の値に変更**して解消（位置停止 0 件、進みの合計が追従時間と一致）。
**測定系そのものを疑う手順が効いた例**として記録する。

### v0.4.3 を公開（2026-09-18 09:35、利用者の許可）

https://github.com/faio1230/timecode-sync-player/releases/tag/v0.4.3 （Pre-release、`v0.4.3 (beta)`）。
setup.exe SHA-256 `AD7B7EB7BE23006D17DAECA9FF34719C5424D40E99D65EDD2443A5D4E2D73057`、
zip `40FEBA7C6B395FE450C70D94986BC68A540E1CEF84D3DB78CD0BE4995DE3CD0A`。

公開の根拠: 検証機の既存 36 本が 3 通しとも 36/36、L-1 の実素材 6 本が 4 合格（残り 1 は着地過渡のみ、1 が D37）、
**公開版 0.4.2 は同じ素材で追従に入れない**（明確な改善）、配布物の起動確認（再生・音声・Spout・LTC 36/36・残プロセス 0）。

### 体感と測定が一致した（2026-09-18 09:20、検証機の利用者の指摘）

**検証機の利用者が画面を見て「引っかかる・飛ぶ」と指摘した素材が M3・M2・M4** で、測定と一致した。

| 素材 | 補正シーク | 絵が止まっていた合計 / 60 秒 | 最長 | L-1 の判定 | 利用者の指摘 |
| --- | ---: | ---: | ---: | --- | --- |
| VP9 4K60 | 7〜15 回 | 5.68〜7.95 秒 | 2,026ms | 失敗 | あり |
| VP9 1080p60 | 2 回 | 0.75 秒 | 706ms | **合格** | あり |
| H.264 1080p60 | 0〜6 回 | 0〜? | 606ms | **合格** | あり |
| ProRes 4K60 | 1 回 | 0.14 秒 | 135ms | 合格 | なし |
| AV1 4K24 ×2 | 0 回 | 0 | — | 合格 | なし |

**L-1 の現行しきい値（2 秒窓の更新 0、誤差 ±0.3 秒）では、0.3〜0.7 秒の固まりが窓に埋もれて通ってしまう。**
境目は「1 回 300〜500ms」または「60 秒あたりの合計 0.7 秒」あたりに見える（**1 回の測定では決めない。M3 を 8 回回して分布で決める**）。

**D37 の定義を広げる**: 「4K60 で連鎖する」ではなく「**補正シークのたびに絵が 0.1〜2 秒止まる**」。
ライブ用途では数値上の誤差より重い。直し方の本命（シーク方式を粗くして着地を速くし、あとは速度補正で詰める）は体感に直接効く。

### L-1 の「更新 0 の窓」に誤検出があった（同日、検証機の指摘）

ある回の `stallUpdateWindows=7` は、**7 件すべてが「その窓に `Playback perf` の行が 1 本も割り当たらなかった」ケース**だった
（`windowsWithoutPerf=7` と完全一致）。アプリが 0 更新と報告したわけではない。
`perf` 行が無い窓は別枠にし、本物の凍結は「perf 行はあるのに位置が進まない窓」で拾う（対応中）。

**あわせて製品側の別事象が見えた**: `Playback perf` の行が 3.36 秒空き、次の行の span が 2.03 秒しかない
（＝アプリの 2 秒タイマーが 1.3 秒滑っている）。「UI スレッドが止まっているのか、CPU が飽和しているのか」を切り分け中。

### D37 の機構が確定した（2026-09-18 10:00、検証機の解析。8 回 ×60 秒、47 件で確認）

**連鎖の条件は「シークの所要時間 > 粗い判定の許容（0.240 秒）」の 1 つだけ。**

#### 機構

シークしている間は**位置が止まり、LTC だけ進む**。したがって

```
着地した瞬間の誤差 ≒ そのシークにかかった時間
```

実測: シーク直後 0.5 秒以内に次のシークが出た 47 件で、**「次の誤差 ÷ 直前のシーク所要」の中央値が 0.99**
（309ms → 0.309 / 1,379ms → 1.390 / 1,012ms → 0.970 / 1,116ms → 1.095 / 254ms → 0.262 …）。

- 所要 > 許容 0.240 秒 → **着地した瞬間にまた条件が成立** → 次のシーク → 自己維持の連鎖
- 所要 < 許容 → 着地後の誤差も許容内なので、そこで止まる

シークの間隔が 0.25 秒前後で揃っているのも、判定周期と整合する。

#### 8 回の「最初の補正シーク」（同じ素材・同じ設定）

| 回 | 総シーク | 停止合計 | 初回の所要 | 次の誤差 | 結果 |
| ---: | ---: | ---: | ---: | ---: | --- |
| 1 | 0 | 0.000s | — | — | きれい |
| 8 | 1 | 0.203s | **203ms** | — | **連鎖せず**（許容の内側） |
| 2 | 11 | 7.068s | 309ms | 0.309 | 連鎖 |
| 6 | 6 | 6.658s | 303ms | 0.327 | 連鎖 |
| 7 | 18 | 10.235s | 632ms | 0.616 | 連鎖 |
| 5 | 16 | 10.897s | 1,267ms | 1.281 | 連鎖 |
| 3 | 9 | 5.358s | 1,116ms | 1.095 | 連鎖 |
| 4 | 6 | 6.392s | 1,472ms | 0.352 | 連鎖（8.7 秒後） |

**追従の入り方には差が無い**（4 回とも開始時の誤差 0.057〜0.097、最初の数窓は更新 120 前後）。
分岐しているのは**最初の補正シークの所要時間だけ**。

#### 直し方の候補（親の整理）

| 案 | 内容 | 見込み |
| --- | --- | --- |
| **A** | **シーク中に進む時間を見込んで目標を先に置く**（`SeekLatencyCompensator`。既定は無効） | 着地時の誤差が消えるので連鎖が切れる。**既存の実装がある**。上限 0.4 秒は 4K60 の 1.5 秒シークに足りない可能性 |
| B | シークを速くする（方式の変更） | 4K60 のロング GOP では accurate である限り限界がある。KEY_UNIT は着地が最大 1 GOP ずれるので同期用途には使いにくい |
| C | 着地後しばらく粗いシークを抑止し、速度補正に任せる | 単独では弱い（誤差 1〜2 秒を ±20% で詰めると 5〜10 秒）。A と組み合わせる前提 |
| D | 許容 0.240 秒を上げる | 連鎖は止まるが、**体感の固まり（500ms 以上が 60%）はそのまま残る**ので対処として弱い |

**本命は A（＋必要なら C）。** 検証機が `TCS_SEEK_LATENCY_COMPENSATION=on` で M3 を 4 回測定中。

> **注意（過去の判定との関係）**: 先行補償は 2026-09-18 07:25 の測定で「60fps のシークが 2.38 秒へ悪化」として既定オフにした。
> ただしその測定は**キーフレーム間隔 1 秒の生成素材**で、シークが 0.2 秒前後と安い条件だった（補償が過剰になる領域）。
> **重い素材では逆に効く可能性がある。** 素材によって効果が逆転するなら、固定の on/off ではなく**実測の所要時間に応じた適応**が要る。

#### L-1 の追加判定（親の決定、2026-09-18）

- **1 回の停止が 500ms を超えたら不合格**
- **追従 60 秒あたりの停止合計が 1.0 秒を超えたら不合格**（秒数が違う run は比例させる）

根拠: 8 回の分布で、体感で問題にならない 2 回は「最長 203ms / 合計 0.203 秒」まで、
引っかかる 6 回は「最長 1,285ms 以上 / 合計 5.36 秒以上」で、**その間に 1 回も入っていない**。

### D37-a の効果と、振動・凍結の出どころの確定（2026-09-18 14:50）

#### D37-a（粗い判定を瞬間値で出さない）を統合（`f259878`）

実機（VP9 4K60、キーフレーム 10 秒の重い素材、トレース有効 3 回）:

| | 修正前 | 修正後 |
| --- | --- | --- |
| 補正シーク | 6〜18 回 | **0 / 0 / 0 回** |
| 停止合計 | 5.36〜10.90 秒 | **0 秒** |
| 最大誤差 | 0.2〜3.8 秒 | **0.100 / 0.107 / 0.107 秒** |
| 平均更新 | 81.6〜99.1 | **120.5〜120.6**（期待 120） |

既存シナリオ 22/22、LTC ループ 14/14、V3 は平均 -28.4ms・p95-p5 40.0ms（前日と同水準）。非E2E 1925。

**残る経路**: トレース無効の参考 3 回のうち 1 回はシーク 7 回。ゲートが初回の偽発火を抑止した後、
**実在の不足が 0.28 → 0.43 → 0.54 → 0.69 → 0.88 → 1.15 → 1.45 秒と増え続けた**もの。これは D37-b の領域。

#### 振動の出どころは「再生位置の問い合わせ」だった（LTC 側ではない）

同じ run で、LTC 側と再生位置側を**それぞれ理想直線と比べた残差の σ**:

| 区間 | LTC 側 σ | **再生位置側 σ** |
| --- | ---: | ---: |
| 平常時（30 秒） | 0.33ms | **0.00ms** |
| **連鎖中（8.8 秒）** | 0.36ms | **445ms**（最小 -781 / 最大 +567ms） |

- **LTC の時刻付けは正しく動いている。** 1 コールバックに届く LTC は 1〜2 枚、
  同一コールバック内の打ち消し（生値 +40ms と age −40ms）は**中央値 0.021ms** で完璧
- アンカー更新による段差は 2240 ペア中 82 件（3.7%）あるが、**76/82 はその場で打ち消されている**
- **「アンカーのずれが振動の原因」という親の仮説は外れ**

**結論: シーク中は再生位置の問い合わせが ±0.5〜0.8 秒の幅で信用できない値を返す。**
その値を判定が拾って次のシークを呼び、連鎖になっていた。

#### 凍結の形は「1 枚で固まる」（コマ落ちではない）

連鎖中 8.8 秒の内訳: `compose.publish` は 60/s で回り続けるが、**実際に変わった絵は 83 枚（9.4 枚/秒）**。

| 停止の長さ | 1.78s | 1.45s | 1.17s | 0.93s | 0.75s | 0.57s |
| --- | --- | --- | --- | --- | --- | --- |
| 回数 | 1 | 1 | 1 | 1 | 1 | 1 |

**合計 6.65 秒＝区間の 75% が完全静止**（同じ 1 枚）。停止と停止の間に十数枚がまとめて出る。
**停止が回を追うごとに伸びている**（自己増幅の可視化）。滑らかなコマ落ちではない。

#### 音声入力の条件（実測で確定）

NAudio `WasapiCapture` の既定のまま: **共有モード / バッファ 100ms / イベント同期なし（ポーリング）**、
形式はデバイスのミックス形式。アプリは何も指定していない（`LtcAudioMonitor.cs:49`）。
共有モードのため、デバイスのサンプルレート変換が挟まりうる（アプリは検知していない）。

### D37-b: シーク中は位置を信用せず、実在の不足は速度補正で詰める（2026-09-18・差し戻し）

エージェント実装 `947796d`（agent-a）。**親判定: 差し戻し（D37-b2 へ）。**
...
**故意に作った 547〜596ms の不足を、シークを 1 回も出さずに速度補正だけで解消した**
（約 97ms/秒で減少、3.2〜3.7 秒で収束）。速度補正の実力は事前の実測（93ms/秒）どおり。
...
**形式的には V3 の不合格ではない**（確定した 4 基準の「収束」は seek-a/b/c/back が対象で、
black-sweep は定常誤差の集計からも明示的に除外されている。本記録 727 行・2326 行）。
**それでも差し戻す理由は 2 つ**:

1. **6〜15 倍の回帰**であり、しかも**過去に直した対象**（D2 で 11,978ms → 51ms、本記録 1029 行付近）。
   基準の外にあるという理由で黙って戻すと、その作業を打ち消す
2. **体感で最も目立つ形の誤差**。黒ギャップ明けは画面に内容が戻る瞬間で、
   そこから数秒間 0.4〜0.7 秒ずれたまま流れる（利用者の指摘と測定が一致した前例がある）

#### 判断: 手段の最適解は「その瞬間に絵が出ているか」で反転する

- **定常の追従中**: 画面に内容が映っている。シークは 0.5〜2 秒の静止を作り、
  静止した時間ぶん誤差が増えて連鎖する（D37 で実証）→ **速度補正が正しい**
- **ギャップ明け・トラック切替の着地**: その瞬間は黒またはフリーズ。**シークの静止は見えない**。
  逆に速度補正に回すと**内容が 0.4〜0.7 秒ずれたまま数秒流れる** → **シークが正しい**

「絵を止めない」ことに価値があるのは**定常の追従中だけ**、という切り分け。
これを D37-b2 として実装させる（`docs/prompts/2026-09-18-D37b2-seek-only-when-picture-is-hidden.md`）。

証跡: `TestResults/x1-d37b/`（l1-trace1..3-*、full-d37b2.trx）、`TestResults/v3/d37b-ltc25-gst/`。

### D37-b2: 「絵を止めない」を定常追従だけに限定した（2026-09-18・親判定 合格水準）

エージェント実装 `814bcd2`（agent-a）。main へ統合 `534bc9c`。

| 項目 | D37-a | D37-b | **D37-b2** |
| --- | ---: | ---: | ---: |
| black-sweep 回復（official / receipt） | 360 / 304ms | 2,079〜5,403 / 2,281〜5,470ms | **412 / 356ms** |
| V3 定常 平均 / p95−p5 | −28.4 / 40.0ms | −27.7 / 40.0ms | **−25.7 / 38.2ms** |
| seek-a/b/c/back・freeze-sweep | 120〜160ms | 120〜160ms | **119〜160ms** |
| L-1 ×3 シーク / 停止 / 最大誤差 | 0 / 0s / 0.100〜0.107 | 0 / 0s / 0.100〜0.103 | **0 / 0s / 0.103〜0.107** |
| 追従開始までの待ち | 9.1〜9.8s | 2.9〜3.3s | **1.84〜1.99s** |

**実装**: `SyncPlaybackState.RateCatchUpAllowed` を追加し、ファイルロード（開始・解除）と
ギャップ出口（黒・フリーズ、`ContinueOnTrackCoordinator`）で着地を通知。**着地から 1.0 秒は速度補正優先を無効化**。
定常の不足は従来どおり速度補正。親の独立検証: ビルド 0 エラー、**非E2E 1949 合格 / 失敗 0**、ロック規則 PASS。

#### 「追従開始までの待ち」は D37-a が作ったものではなかった

D37-b の報告で 9.3〜9.7 秒 → 2.9〜3.3 秒と出たため、**D37-a が 3 秒の待ちを増やしたのではないか**を確認させた。

- **D37-a 前が 9.1〜9.8 秒、D37-a 後が 9.2〜9.6 秒で同帯**。増やしていない
- 支配項は**初回キャッチアップのシーク連鎖**（D37-a 前の run2 で 7 回、D37-a run1 で 8 回・7.7 秒）。
  D37-a のゲート遅延（3 サンプル ≈ 120ms/回）はその中に埋もれる
- したがって 0.4.3 に対しては**改善**（9 秒台 → 2 秒前後）。リリースノートに制限として書く必要はない
- 留保: D37-a 前のサンプルは 2 本、D37-a run3 は 0.464 秒で run ごとのばらつきが大きい

### L-1 の監査区間に測定側の不具合があった（検証機の発見、2026-09-18）

検証機が発見・修正（`d962383`）。**過去の「最大誤差」の数字に波及する。**

信号は `Play(startLtc, track.Used - startLtc - 1.0)` で送出するのに、監査区間の上限を
**`track.MediaOut` 基準**で計算していた。基準のずれは **`MediaIn + 1.0` 秒**。
ゲート待ちで LTC を消費すると、**信号が尽きた区間まで測ってしまう**。

| | 修正前 | 修正後 |
| --- | ---: | ---: |
| 追従 | 59.2 秒 / 29 窓（最後の窓は `ltcAdvance=0.000`） | 53.16 秒 / 26 窓（全窓で LTC が進む） |
| 最大誤差 | **2.957**（別の回 2.84） | **0.187** |

**波及**: 本記録および指示書で「修正前の最大誤差 0.2〜3.8 秒」と書いた数字のうち、
**実素材（`MediaIn` > 0）の回の上限側は測定側の混入を含む**。**製品の誤差ではない。**
一方、L-1 のしきい値の根拠に使った 8 回の分布は**停止時間ベース**なので影響を受けない
（単発 500ms / 60 秒あたり合計 1.0 秒はそのまま有効）。

**教訓**: 時間軸の基準（素材時間 / タイムライン時間 / 信号の送出範囲）が混ざる箇所は、
**必ず同じ基準に揃っているかを検算する**。同種の取り違えは T5 の「タイムライン秒 vs 素材秒」でも起きている。

## 0.4.4 候補の検証機判定: **公開を保留**（2026-09-18 19:20、親）

候補 `8703f4f`（setup.exe SHA-256 `C0D2…E5E9`）。検証機で標準セット（3 通し + L-1 5 本）と M3 ×8 を実施。

### D37 の狙いは実素材でも達成した（ここは変えない）

M3（VP9 4K60 ロング GOP）×8 回、同条件:

| | 0.4.3 | **0.4.4 候補** |
| --- | --- | --- |
| 補正シーク | 0, 1, 6, 6, 9, 11, 16, 18 回 | **すべて 0 回** |
| 停止合計 | 0〜10.897 秒 | **すべて 0.000 秒** |
| 最長の停止 | 0〜2,038ms | **0ms** |

シナリオ 3 通しは 36 合格 / 0 失敗 ×3。L-1 は 5 本中 4 本合格。

### 保留の理由 1: 追従開始時に 1.8 秒ずれ、解消に 18〜22 秒かかる

**L-1 の「合格 6 回・失敗 2 回」は、同じ現象の見え方の違いだった。**

| | ゲート待ち | ゲート時の誤差 | 窓 0 の誤差 | 最大誤差 | 0.3 未満への収束 |
| --- | ---: | ---: | ---: | ---: | --- |
| 6 回 | 17.56〜19.73s | 0.217〜0.297 | 0.207〜0.243 | 同左 | 窓 0（即） |
| 2 回 | 0.55 / 0.59s | 0.033 / 0.040 | 1.807 / 1.813 | 同左 | 窓 10 / 11（20〜22 秒） |

6 回は**テスト側のゲートが 18 秒待ち切ってから測り始めた**ので、その 18 秒が測定範囲の外に落ちていた。
**合格した回だけ見ていたら、この 1.8 秒は見つかっていない。**

#### 機構（確定）

M3 は 0.4.3 でシーク所要が**最長 2,038ms** だった素材。`LearnedSeekDurationSeconds` が約 2 秒に学習されると、
速度補正を優先するしきい値（`_rateCatchUpLimitSeconds`）も約 2 秒になる。**1.8 < 2.0 なので速度補正が選ばれる。**

1.8 ÷ 0.097 = **18.6 秒**で、実測の 17.6〜19.7 秒と一致する。

**ルール自体は正しい**（誤差 < シーク所要でシークすると、直すより大きい誤差を作る）。
**しかし追従開始でそれをやると、18 秒間ずっと映像が LTC より約 10% 速く流れる。** シーク 1 回（2 秒）の方がよい。

**親の設計の穴**: D37-b2 で「ギャップ明け・トラック切替の着地はシークで詰める」と決めた理由は
「**その瞬間は画面が黒かフリーズなのでシークによる静止が見えない**」こと。
**追従開始も同じ性質を持つのに、入れ忘れていた。**

### 保留の理由 2: 振動が速度補正の経路に移り、振幅はむしろ増大

検証機の観測（標準セットの M3、追従 42.1 秒）:

- **Seek decision gate の棄却 67 回**（新ゲートが不安定サンプルを弾き、シークを 0 回に抑えている。**狙いどおり**）
- **Smooth correction 232 回。残差の絶対値 中央 60ms・最大 754ms・300ms 超が 34 回**
- **rate が 1.10 と 0.95 の間を 50ms ごとに振れ続ける**

| | 0.4.3 | 0.4.4 候補 |
| --- | --- | --- |
| 残差の振幅 | ±100〜170ms | **±300〜754ms** |

**親の設計の穴**: D37-a の指示書に「**Jump 補正と Smooth は触らない。対象は粗い判定だけ**」と明記した。
**同じ汚れた入力が速度補正に入り続けることを考えていなかった。**
ゲートはシーク経路だけを守り、速度補正は生の値を使っている。

### 撤回した記述

**「追従開始までの待ちが 9 秒台 → 2 秒前後に改善」をリリースノートと CHANGELOG から撤回した**（`014bbf8`）。
**開発機の生成素材での話であり、実素材の M3 では 7.02 秒 → 18.05 秒に悪化していた。**
測定した条件の範囲を超えて主張した、この日 4 回目の同型の誤り。

### 対応

**D37-c**（`docs/prompts/2026-09-18-D37c-follow-start-and-rate-path.md`）:

1. **追従開始を着地窓に入れる**（誤差の大きさに関係なく、着地窓の中ではシークを選ぶ順序であること）
2. **速度補正の入力にも、粗い判定と同じ前処理を通す**（ありえない標本の除外＋中央値）。
   **中央値は反応を鈍らせるので、V3 の収束（1 秒以内に ±80ms）の非回帰を必ず確認する**

**D37-c は対症療法であり、根治は 0.4.5-A**（位置フィードバックの一貫化）。
振動が完全に消えなくてよく、**速度補正が上下限に張り付かなくなれば目的を達する**。

### 検証機に依頼した体感の確認（判定の材料）

1. rate が ±10% で 20Hz 振れているのが**目に見えるか**（ざわつき）
2. 再生開始から 18 秒、映像が LTC より約 10% 速く流れ続けるのが**目に見えるか**（じわっと追いかける）

**2 の方が気づかれにくく、しかも 18 秒続くので実害が大きい可能性がある。**
どちらがどう見えるかで D37-c の合格線と優先順位が決まる。

### テスト側の課題（検証機の担当）

ハーネスのゲートが**瞬間値 1 点**で判定しているため、8 回中 2 回、実際の誤差 1.8 秒のところを
0.033 / 0.040 と読んで通過した。粗い判定と同じ「中央値か N 回連続」が要る。

**同じことが製品にも言える**: UI の位置表示も同じ不安定な値を出しているので、
**人が画面を見ても 1.8 秒ずれていることが分からない**。これは 0.4.5-A の対象。

## M5 が V3 の数字の読み方を変える（2026-09-18 20:10、親）

M5（`docs/analysis/2026-09-18-M5-ltc-video-fps-ratio-result.md`）で、
**LTC fps と映像 fps の組み合わせが、ばらつきの下限（量子化の床）を決める**ことが分かった。

| LTC × 映像 | 比 | p95−p5 の床 |
| --- | --- | ---: |
| 24 × 24 / 25 × 25 | 1:1 | **0.00ms** |
| 30 × 29.97 | ≈1:1 | 7.70 |
| 30 × 60 | 1:2 | 16.67 |
| 25 × 60 | 12:5 | 26.67 |
| 25 × 24 | 24:25 | 41.67 |

### V3 のフィクスチャはすべて非 1:1 である

V3 は **LTC 24/25/29.97/30 × 映像 24 / 29.97 / 60fps**。**1:1 になる組み合わせは事実上含まれない**
（LTC24 × 映像24 のセルだけが 1:1 だが、3 クリップのうち 1 つ）。

**したがって V3 の合格記録「p95−p5 50〜67ms」には、量子化の床（最大 41.67ms）が含まれている。**

### 帰結

1. **同期制御そのものの性能は、V3 の生の数字が示すより良い。**
   測定値から床を引いた残りが制御の寄与であり、それは 50〜67ms よりかなり小さい
2. **0.4.5-A の目標を「p95−p5 をさらに下げる」に置いてはいけない。**
   組み合わせごとの床より下には原理的に行けない。
   **0.4.5-A が狙うのはシーク中の誤差の可視化であって、定常のばらつきではない**
3. **組み合わせをまたいだ比較をしない。** 0.4.5-A の前後は**同じ LTC fps・同じフィクスチャ**で比べる
4. **V3 の合格基準（p95−p5 ≤ 100ms）は変えない。** 固定のフィクスチャに対する一貫した指標であり、
   床が含まれることを承知のうえで使う分には有効

### 未解決

`29.97 × 29.97` は 1:1 なのに p95−p5 が 33.37ms ある（他の 1:1 は 0.00）。
**NTSC 系の現場は 29.97 が標準**なので、実務上の重要度が高い。調査中。

## D37-c の検証機判定（2026-09-18 20:45）: 穴 1・穴 2 とも解消。**G-5 の再現率待ちで保留**

候補 2（`0.4.4+68b749c`、SHA-256 `12EC…C80F`）。テストツリーも `68b749c` に固定して実施。

### 穴 2 は解消した。しかも**前処理が効いたのではなく、入力が綺麗になった**

M3（追従 56.4 秒）の Smooth 残差:
...
| 最大 | **754ms** | **188ms** |
| 300ms 超 | **34 回** | **0 回** |
| rate 1.10 への張り付き | 張り付き | 3 回のみ（0.95 以下が 20 回） |

**決定的なのは `rawResidualMs`（前処理前）も 中央 20ms・最大 188ms・300ms 超 0 回だったこと。**
**前処理が汚れを隠しているのではなく、入力そのものが綺麗になっている。**

**理由**: 穴 1 の修正（追従開始をシークで着地）により、**追従開始の長い速度補正の区間そのものが消えた**。
その区間が位置クエリの暴れる場所だった（0.4.5-A の観測: シーク中 `playback` が目標で凍結し誤差が育つ）。
**前処理は保険として残るが、主因は穴 1 だった。**

棄却 27 回、`positionFallback` 0 件。他の 4 素材は Smooth 9〜35 回・残差最大 8〜35ms・張り付き 0 回（元から問題が出ない素材）。

### 穴 1 は解消した

| 素材 | ゲート待ち | 0.4.4 候補 1 | シーク | 最大誤差 | 判定 |
| --- | ---: | ---: | ---: | ---: | --- |
| **M3** | **3.77s** | **18.47〜19.73s** | 0 | 0.197 | 合格 |
| M5 | 1.84s | 1.31s | 0 | 0.102 | 合格 |
| M2 | 1.84s | 1.18〜2.29s | 0 | 0.180 | 合格 |
| M4 | 2.08s | 0.52s | 0 | 0.233 | 合格 |
| M1 | 1.60s | 0.97s | 0 | 0.107 | 合格 |

**事前登録した上限 5 秒に収まった。** 5 本ともシーク 0 回・停止合計 0 秒。

### 保留の理由: G-5 が 1 件失敗

`pass-b` の `G5_ContinueBlack_JumpCycleAroundTracksAndGaps_KeepsExpectedPictures`（g5-03）。
**症状は「位置は正しいが絵が黒」。**

```
20:11:18.312  Continue mode: switching to track M3 at media position 14.881s
20:11:18.729  Gst loadfile（M3、start=14.881、paused=true、rc=0、elapsedMs=416.2）
20:11:19.393  判定: position=14.983（期待 15）だが isBlack=true / blackFraction=1.0
```

**切替の決定から 1.08 秒、ロード完了から 0.66 秒たっても 4K60 の最初の絵が出ていない。**

**回帰を疑う根拠**: **候補 1 では 3 通しとも 36 合格 / 0 失敗**だった。
検証機は「D34 期から断続的に落ちていた項目」として単発の可能性も見ており、**4 回の再現試験を実施中**。

**親の仮説（未検証）**: D37-c は追従開始を着地窓に載せた。トラック切替の直後は
**D37-b2 の切替着地窓と D37-c の追従開始着地窓が重なる**可能性があり、
**ロード直後に余分なシークが出て、読み込んだばかりのフレームをフラッシュしている**としたら症状と整合する。
切替からの 1.08 秒に `seek.issue` があるかで潰せる。

### 観測性についての合意

**「shim 側にカウンタが無い項目は『観測できていない』と書く」**を検証機と合意した。
`gst.positionFallback` が「痕跡 0」から「実際には発火していた」に変わった件の一般化。
**「観測されていない」と「起きていない」を取り違えないため。**

### G-5 の判定（2026-09-18 21:00、親）: **D37-c の回帰ではない。0.4.4 の阻害要因から外す**

**親の仮説（着地窓の二重適用で余分なシークが出てフレームをフラッシュ）は否定された。**
失敗した回の「切替 → 判定」1.2 秒間のトレースで **`seek.issue` / `seek.decide` / `seek.return` はいずれも 0 件**。
`gst.delivery` は +0.209 秒から来ており、配信自体は始まっている。

#### 実態は 0.13 秒のタイミング競合

| | ロード | 判定時刻 | 結果 |
| --- | ---: | ---: | --- |
| 失敗した回 | +0.42s | **+1.081s** | 黒 |
| 合格した回 | +0.43s | **+1.214s** | 黒でない |

**4K60 の切替後、最初の絵が出るまでロードから 0.7〜0.8 秒**かかり、
テストは 1 回だけ撮るためその境目で黒を掴むことがある。**再現率は 5 回中 1 回**（4 回連続で合格）。

**テスト側の修正**（検証機の担当）: 保持中の判定を「黒でない絵が出るまで短時間待つ」形にする。判定の意味は変えない。

#### 製品側に残る事実（0.4.5 以降の課題として記録）

**トラック切替から最初の絵が出るまで 0.7〜0.8 秒**（ロード 0.42 秒 + 最初の絵まで 0.3〜0.4 秒、実素材 4K60）。

- D37-b2 で測った「ギャップ明けの回復 412ms」は**生成素材の値**。**実素材の 4K60 では別の数字**
- G-5 はギャップモード Black の切替なので、この 0.7〜0.8 秒は**元から黒の区間**に入る
- **ただしギャップを挟まない切替では、凍結した絵が 0.7〜0.8 秒残る形で見える。** そちらの方が現場では目立つ
- **4K60 のロード時間を詰めるときの基準値とする**

#### 観測性の書き分け（検証機と合意した方針の実例）

- `seek.issue` は**トレースにカウンタがあり 0 件を観測** → 「起きていない」と言える
- 「最初の絵まで 0.7〜0.8 秒」は**カウンタが無く、0.66 秒では黒・0.78 秒では黒でない、という 2 点で挟んだ推定**

**前者は仮説を潰せるが、後者は潰せない。** この書き分けが無いと「観測されていない」を「起きていない」と
取り違える。同日 `gst.positionFallback` でその取り違えを踏みかけた。

### 0.4.4 の公開阻害要因は **D37-d のみ**になった（2026-09-18 21:00）
