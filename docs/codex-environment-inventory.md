...
- ユーザー共通スキル27件の無効化は、プロジェクト設定では診断出力に反映されなかった。同じ指定は CLI 上書きで反映された。共通設定での無効化は、他プロジェクトにも影響するため確認待ち。

確認には PATH 上の Codex CLI 0.144.6 とアプリ同梱の 0.153.4 を使用した。`debug prompt-input` では、プロジェクト設定によりプラグイン由来スキルが消え、ユーザースキル27件とシステムスキル5件が残ることを確認した。設定値の読み取りでも `plugins` / `remote_plugin` / `apps` / `hooks` が false であることを確認した。
