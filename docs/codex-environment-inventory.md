...
- ユーザーの希望に合わせ、共通スキルは重複11件だけ無効化した。`~/.agents/skills/` の16種類を有効のまま残し、`~/.codex/skills/` の重複コピー11件を共通 config.toml の `[[skills.config]]` で無効化した。27件すべてを無効化する案は採用していない。ファイルは両方の保存先に残している。
...
共通設定変更前のバックアップは `~/.codex/config.toml.bak-skill-dedup-20260908-220039-355729`。重複を戻す場合は、共通 config.toml に追加した11件の無効化指定を除く。共通設定のため他プロジェクトでも片方のコピーに統一されるが、スキル16種類は引き続き使用できる。
