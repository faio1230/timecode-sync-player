...
## マスター goal（T1〜T7 一括実行用）

T1〜T7 を一括で自律実行させる場合は以下をそのまま goal に使う。各タスクの詳細指示は
本ドキュメントの該当セクションを participating エージェント自身に読ませる構成のため、
プロンプト本体は制御ルールに徹している。

```
リポジトリ: timecode-sync-player（Windows WPF / net8.0-windows）
【最重要】Linux のクラウド実行環境ではビルド不可。Windows ローカルで実行すること。

goal: docs/test-coverage-roadmap.md に定義されたテストカバレッジ拡充タスクを
T1 → T2 → T4 → T3 → T5 → T6 → T7 の順に実行する。まず同ドキュメントを全文読むこと。
各タスクの詳細要件・テスト観点・抽出方針は各セクションの goal プロンプトに
記載されている。それに従う。
...
T7 はロードマップ上「対話セッション推奨」だが、本 goal では以下の縮小ルールで
可能な範囲のみ着手する:
