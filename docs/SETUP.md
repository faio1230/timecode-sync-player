...
| モード | 挙動 |
|---|---|
| **Smooth**（既定） | 残差に比例して再生レートを動かします（`rate = 1 + clamp(e / 1.0s, ±0.10)`）。**シークを発行しない**ので絵は飛びません。シーク後の着地誤差（100〜150ms）は約 1.5 秒で詰めます。GStreamer のパイプラインが `INSTANT_RATE_CHANGE` に非対応の場合は使用不可となり、その状態を表示します（**Jump へ切り替えて**ください。フラッシュシークへの自動フォールバックはしません）。レートを出しても残差が縮まない場合も、既定回数で諦めて警告し、補正なしへ落ちます |
