...
| モード | 挙動 |
|---|---|
| **Smooth**（既定） | 残差に比例して再生レートを動かします（`rate = 1 + clamp(e / 1.0s, ±0.10)`）。**シークやトラック切替の着地直後 1.0 秒だけ上限が ±0.20** になります（収束を 1 秒以内にするため。着地誤差 100〜150ms なら約 0.5〜0.8 秒で詰まります）。**シークを発行しない**ので絵は飛びません。GStreamer のパイプラインが `INSTANT_RATE_CHANGE` に非対応の場合は使用不可となり、その状態を表示します（**Jump へ切り替えて**ください。フラッシュシークへの自動フォールバックはしません）。レートを出しても残差が縮まない場合も、既定回数で諦めて警告し、補正なしへ落ちます |
| `Jump` | 残差が **80ms（LTC 2 フレーム分）** を超えたらフラッシュシークで合わせます。デッドバンド 20ms より広いのは、LTC の粒度と音声コールバック（50ms ごと）で残差に ±20〜40ms の揺れが乗るためです。連続 3 回で止まり、残差が 1 秒間しきい値の内側に留まると回数が戻ります（揺れで飛び続けない） |
