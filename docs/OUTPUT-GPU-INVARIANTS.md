...
| I12 | ~~**Cpu backend は不変**~~ **段 3（2026-09-16）で失効。** CPU 合成は除去する（利用者の決定 2026-09-14）。段 3 の完了までは触らない。 | 計画 → v0.4 |
...
| I14 | **サンプルは flush 境界で世代に属する**: シークの flush 前にストリーミング側へ引き込まれたサンプルを、シーク後の世代として公開しない。境界は「シーク後の SEGMENT がシンクのパッドを通過したか」で決め、通過前のタグを持つサンプルは破棄する。世代（`generation`）の刻印だけでは不十分（引き込み済みサンプルの処理がシーク準備に遅れると新世代で刻印される）。 | D25（2026-09-17、V5 の 1/10・E2E の 8/22） |
| I15 | **共有フェンスの値は再利用しない**: lease seq（＝共有フェンスの signal 値）はフェンス資源の寿命内で厳密に増加させる。ロード／パイプライン再構築でリセットしない（リングとフェンスは同一寸法のロードをまたいで残るため、値が巻き戻ると `IsFenceComplete(seq)` がコピー完了前に真になる）。統計用のフレーム数はロード毎でよいが、フェンス値とは別に数える。 | D25（2026-09-17、一時停止シーク直後の「PTS は目標・画素は前」） |
