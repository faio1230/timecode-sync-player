...
| 用途 | パス |
| --- | --- |
| 親（runner・集計・結果） | `C:\Users\<user>\Documents\timecode-sync-player\.superpowers\worktrees\session-refactor`（branch `refactor/session-lifecycle`） |
| アプリ実行対象（runner 既定 `-AppExe`） | `C:\Users\<user>\Documents\timecode-sync-player-wt-verify-oe-20260911-1344`（a9b9349、ビルド済み） |
...
```powershell
$runner = 'C:\Users\<user>\Documents\timecode-sync-player\.superpowers\worktrees\session-refactor\TestResults\gpu-mutex-retry-session-20260910T0752Z\Invoke-AppGpuTrial.ps1'
