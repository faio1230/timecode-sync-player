...
- **次の 3 手**:
  1. v0.5.3 の候補 `0.5.2+7e0d7d5` を検証機へ送った（2026-09-26 10:55、固定の一式 2 回を依頼）。開発機の検証は済み（[release-0.5-plan.md](release-0.5-plan.md) の「v0.5.3 候補」、前後比較は d38 文書の §9）。
     合格したら版を 0.5.3 に上げ、リリースノート（既知の制限に D39 の型 (2)＝読み直したプロジェクトのオフセットが 0 になることがある）→ main へ --no-ff → タグ → 公開の許可で止まる
