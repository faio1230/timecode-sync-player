...
読み取り調査と、ローカルの GStreamer 1.28.2（MSVC x64、`C:\Program Files\gstreamer\1.0\msvc_x86_64`）での短い動作確認。本体・試作のコード変更はしていない。テスト素材は ImageMagick 同梱の ffmpeg（`--enable-libsnappy`）で生成し、`C:\Users\<user>\AppData\Local\Temp\happrobe\` に置いた。
...
gst-launch-1.0 -v filesrc location=C:/Users/<user>/AppData/Local/Temp/happrobe/hap1_1080p.mov ! qtdemux ! fakesink
