...
| `TCS_HAP=on` | off | v0.5.0, experimental. Plays HAP files by decompressing them on the GPU (Hap, Hap Alpha, Hap Q). Without it, HAP files fail to load. Hap 7 and Hap HDR are refused with a reason; Hap Q Alpha is not guaranteed until verified with real footage / v0.5.0、試験的。HAP の素材を GPU で展開して再生する（Hap・Hap Alpha・Hap Q）。設定しないと HAP の素材は読み込みに失敗する。Hap 7・Hap HDR は理由を出して断る。Hap Q Alpha は実素材で確かめるまで未保証 |
