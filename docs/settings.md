...
| `TCS_HAP=off` | on | Turns off HAP playback (v0.5.0 plays Hap, Hap Alpha and Hap Q by decompressing them on the GPU). While off, HAP files fail to load; they are never decoded on the CPU. Hap 7 and Hap HDR are always refused with a reason; Hap Q Alpha is not guaranteed until verified with real footage / HAP の再生を止める（v0.5.0 は Hap・Hap Alpha・Hap Q を GPU で展開して再生する）。止めている間は HAP の素材は読み込みに失敗する（CPU では展開しない）。Hap 7・Hap HDR は常に理由を出して断る。Hap Q Alpha は実素材で確かめるまで未保証 |
| `TCS_SWITCH_LOAD_LEAD=off` | on | Turns off the switch-load lead (v0.5.1). In Continue, a track switch loads the new track ahead of the timecode by the time its loads have taken before (median of the last 5 switch loads of that track; the first, cold load is not used), so the first frame lines up with the timecode. Off loads at the timecode position and catches up afterwards, as in v0.5.0 / トラック切替の先回りを止める（v0.5.1）。Continue でトラックを切り替えるとき、そのトラックの過去の切替の読み込み所要（直近 5 回の中央値。冷えた 1 回目は使わない）だけ先から読み込み、最初の絵をタイムコードに合わせる。止めると v0.5.0 と同じくタイムコードの位置から読み込み、後から追い付く |
