...
- 2026-09-17 13:08: **D26 / D26-b を main に統合**（ジャンプ中の黒 0 枚、G 6 本・F-1〜F-3 合格、V5 10/10、V6 短縮で WS 安定）。F-4 / F-5 の間欠失敗は D25（shim: シーク直後の古いフレーム）に帰着。項目 27 合格。次: 同期担当 = D27-b の実機 → D24/D25（shim）、除去担当 = D28（合成の完了待ち）+ S-1 のテスト観測間隔 + 参照採取の頑健化
...
- 2026-09-17 01:10: **D27-c を main に統合**（S-2 ×3 成功、R 系と LTC ループ 14 本は不変）。D28・S-1 の判定変更・参照採取の取り直しも統合済み。**製品側の残件は shim の D25（最優先）と D24 のみ**。参照採取・Freeze・R-2 の間欠はすべて D25 に帰着

- 2026-09-17 07:40: **D24・D25・D25-b を main に統合**（参照採取の古いフレームは消失、V5 10/10、V3 -30.6 / 35.7）。残件: D25-c（一時停止中の位置クエリを映像 PTS に、shim）、D27-d（停止時の着地目標を保持値に）、S-1（テストの写像）。これらの後に配布物

- 2026-09-17 10:30: **D25-c を統合。製品側の既知の欠陥はすべて main に入った**（D24/D25/D25-b/D25-c、D26/D26-b、D27〜D27-d、D28、D20〜D22、D21-b、D20-b）。次: 除去担当（新セッション）が統合後 main で シナリオ 22 + LTC ループ 14 + V4 + E2E 全件 → 親が配布物を作って Taildrop で検証機へ → 実素材（`-Media M1,M4,M6 -MediaInOffsetSeconds 5`、`M1,M3,M5`）→ 合格なら 0.4.3

- 2026-09-17 11:30 開発機（統合後 main、除去担当の実機）: シナリオ 21/22（C-2 で B → A ジャンプ中に 1 tick の黒 = **D26-c**、間欠）、LTC ループ 14/14、V4 1/1、**E2E 全件 87/0/6**。配布物を作って検証機へ（D26-c は並行調査）

- 2026-09-17 12:30: 候補ビルド（**0.4.2 のまま**、ProductVersion `0.4.2+c3f3cd3…`）を作成し Taildrop で検証機へ送付。`TimecodeSyncPlayer-v0.4.2-c3f3cd3-setup.exe` SHA-256 `867940E67A1BDD3D2EABCF157C559FD124C9FF0729231331B2AE2400959547E8`（38,659,554）、zip `BB8A555C8EF444B57D3993B87EF40A3CFF9064E2DD2E2205C6D0D164E6D0B1BC`（17,740,087）。プラグイン 19、mpv 0、Release shim ハッシュ一致。展開ビルドの起動確認: 44.1kHz 素材ロード 176.4ms、`already playing`、送信 60/秒、ERR 0、exit 0。検証機には `-Media M1,M4,M6` / `M1,M3,M5` / `M2,M1,M4`（各 `-MediaInOffsetSeconds 5`）の実素材実行を依頼

- 2026-09-17 15:30: 2 本目の候補ビルド（**0.4.2 のまま**、ProductVersion `0.4.2+0a30ad6…`、D29・D30 込み）を Taildrop で検証機へ送付。`TimecodeSyncPlayer-v0.4.2-0a30ad6-setup.exe` SHA-256 `57D75B6A79D139CEA3368568DDA02B8182A6AECAB705D20D936B6C0B82CFE2BF`（38,664,671）、zip `1549A183DD1E1630B6BD21477BF557BC8EB41AA677D52B2298B4E1EA217DF74A`（17,741,062）。プラグイン 19、mpv 0、Release shim ハッシュ一致。検証機には c3f3cd3 の 3 回の報告後に同じ 3 回を再実行して差分を報告するよう依頼。除去担当は統合後 main でシナリオ 22 + E2E 全件を実行中

- 2026-09-17 16:10 検証機（候補 1 `c3f3cd3`、実素材 3 回）: a 25/12、b 27/10、c 29/8。**クラッシュ・完了待ち超過・pump deadline は 0**、LTC ループ 14/14 ×3。失敗の分類は `docs/GSTREAMER-GPU-VALIDATION-PLAN-2026-09-12.md` の「候補 1 の結果と分類」（D29/D30 で直る見込み、長 GOP のシーク遅延 = 仕様上の制約、4K の切替・追従の遅れ = 実測待ち、Freeze の実素材での不一致、テスト側 2 件）。候補 2（`0a30ad6`）を検証機が実行中
...
#### 4.5.x 統合後 main の間欠 2 件の解析（2026-09-17 18:25、除去担当の解析を親が判定）
...
#### 4.5.y 検証機の候補 2（0.4.2+0a30ad6）の実素材結果（2026-09-17 19:00）

a 24/13、b 24/13、c 27/10（候補 1 は 25/27/29）。改善せず。分類と依頼は `docs/GSTREAMER-GPU-VALIDATION-PLAN-2026-09-12.md` の「候補 2」節。テスト基盤側 3 件（S 系への 1 時間台 LTC の混入、メタデータ待ちの時間切れ、参照 PNG の古さ）は検証機で修正、製品側の疑い（F-3/F-4/F-5 の絵が更新されない、F-1 の終端手前保持、S-3 c、C-1/C-2 の黒、G-2、S-1）は証跡待ち。D31/D31-b は開発機で進行中。
- 2026-09-17 20:10: **D31/D31-b（同期担当）と C-1 区間判定（除去担当）を統合（`cb11b0f`）**。統合後 main でシナリオ 22/22・LTC ループ 14/14・RealProjectGap 1/1。候補 3（`0.4.2+4469cdd`）を検証機へ送付。A1 の机上解析を `docs/analysis/` に記録、4K 生成素材で開発機再現へ
- 2026-09-17 20:24: **D32 を統合（`adc8200`）**。統合後 main でシナリオ 22/22・LTC ループ 14/14。候補 4 を検証機へ。D33（Single の MediaOut ホールド、`docs/prompts/2026-09-17-D33-…`）を同期担当が実装中。テスト側は S-3 の期待値を [MediaIn, MediaOut] で clamp（除去担当）
- 2026-09-17 20:31: 検証機の候補 3（4469cdd、ミュート起動）: **a 31/36、b 31/36、c 32/36**（候補 2 は 24/24/27）。残り: S-3/S-5（D33）、F-5（ほぼ黒の冒頭参照、テスト側）、C-1 a（黒から始まるジャンプ、テスト側）、G-2 a（製品側の疑い、証跡待ち）、間欠 4 件。詳細は `docs/GSTREAMER-GPU-VALIDATION-PLAN-2026-09-12.md` の候補 3 節
- 2026-09-17 20:41: テスト側 3 件を統合（`841ef3e`、除去担当 `df6753d` / `57f2f3f`）: 参照どうしの距離が閾値未満なら同定不能（reference-ambiguous、失敗にしない）、参照採取はシーク後に位置が目標 ±1 フレームに入るまで最大 3 秒待つ（絵の変化は早抜けのみ）、ジャンプ発行直前の絵が黒なら jump-black を数えない。非E2E 1859/0。D33 は同期担当が実機中
- 2026-09-18 04:04: 検証機の候補 5（3a122b5、D32+D33）: **a 35/36、b 34/36、c 34/36**。製品側の疑いは解消。残りは参照採取の待ち（テスト側、検証機で修正）、metadata-wait 1 件、M2 長 GOP の間欠 2 件。再実行後に 0.4.3 へ
