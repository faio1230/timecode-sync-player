...
- プロジェクトの生成: `scripts/make-ltc-scenario-project.ps1 -MediaDir <素材フォルダ> -Out <.tsp> [-Tracks 3] [-SegmentSeconds 20]`。フォルダ内の動画（拡張子 mp4 / mov / mkv / mxf / ts）を名前順に先頭から `-Tracks` 本、各 `MediaIn 0`・`MediaOut SegmentSeconds`、先頭オフセット 5 秒、ギャップ 5 秒で並べる。`ffprobe` で尺を読み、`SegmentSeconds` より短い素材は尺どおり。**`.tsp` は素材フォルダに書かない**（検証機の指摘 2026-09-17 10:45: 素材フォルダは利用者が管理する読み取り専用の場所で、ファイル名が残る `.tsp` を置くと素材と一緒に持ち出される）。`-Out` は `ReportDir` 配下（git-ignore 済み）に置く。**製品の `ProjectSerializer.ResolvePath` はプロジェクトディレクトリ外の素材を拒否する**（初回コミットからの規則。絶対パスも `..` も不可）ので、絶対パス参照は使えない。代わりにランナーが **`ReportDir\media` にディレクトリジャンクション（`mklink /J`、管理者不要）を作って `MediaDir` を指し**、生成スクリプトにはジャンクション経由のパスを渡す。`.tsp` の素材参照は `.tsp` のディレクトリからの相対（`media\<ファイル>`）になり、`Path.GetFullPath` はジャンクションを辿らないため製品の規則を満たす。素材フォルダにもその親にも書かない。実行後にランナーは `.tsp` とジャンクションを消す（ジャンクションは `cmd /c rmdir` でリンクだけ消す。PowerShell の `Remove-Item` は実体を消す恐れがあるので使わない。`-KeepProject` で残せる）
