...
- 2026-09-18 11:30 開発機（統合後 main、除去担当の実機）: シナリオ 21/22（C-2 で B → A ジャンプ中に 1 tick の黒 = **D26-c**、間欠）、LTC ループ 14/14、V4 1/1、**E2E 全件 87/0/6**。配布物を作って検証機へ（D26-c は並行調査）
...
- 2026-09-18 16:10 検証機（候補 1 `c3f3cd3`、実素材 3 回）: a 25/12、b 27/10、c 29/8。**クラッシュ・完了待ち超過・pump deadline は 0**、LTC ループ 14/14 ×3。失敗の分類は `docs/GSTREAMER-GPU-VALIDATION-PLAN-2026-09-12.md` の「候補 1 の結果と分類」（D29/D30 で直る見込み、長 GOP のシーク遅延 = 仕様上の制約、4K の切替・追従の遅れ = 実測待ち、Freeze の実素材での不一致、テスト側 2 件）。候補 2（`0a30ad6`）を検証機が実行中
...
#### 4.5.x 統合後 main の間欠 2 件の解析（2026-09-17 18:30、除去担当の解析を親が判定）

| 件 | 機構 | 当たり | 対応 |
| --- | --- | --- | --- |
| S-2 s2-19（保持 8.0 への着地不成立、位置 20.000） | D30 の未確認 Jump（detected-fps）の確認窓が壁時計（音声コールバックで打つ `TickCount64`）の差 ≤100ms。WASAPI 捕捉が滞ると次フレームが 203ms 後にバーストで届き確認不成立 → 保留破棄。以後は保持 Duplicate のみで D27-b の Jump 復帰も無音 5 枚復帰も成立しない | **製品側** | D31: 確認窓をサンプル時計（`FrameEndTimestamp`）で取る。`docs/prompts/2026-09-17-D31-jump-confirmation-by-sample-clock.md`（同期担当） |
| C-1 c1-01（着地は成立、position=6.300） | ランスルー（既定）は保持中も動画が進む。`CheckHold` の着地判定は固定目標 ±0.3 を 50ms 間隔で見るため、窓に居る約 70ms を外すことがある | **テスト側** | 期待を「目標 + 保持開始からの経過秒」に（停止モードは固定のまま）。`docs/prompts/2026-09-17-C1-landing-judgment-under-run-through.md`（除去担当） |
