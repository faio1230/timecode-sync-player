...
# 結合・システム層（重点。単体タスクより優先度高）

単体は既に厚い一方、**「複数コンポーネントを合成した状態遷移・組み合わせ」を検証する層が薄い**。
現状の結合層は E2E 37件（単機能のスモーク中心）と実機 LTC ループのみで、
「オペレーターが実際に行う操作列」や「状態機械3系統の合成」を通しで検証するテストがない。

## V8: ヘッドレス結合シナリオハーネス（最重要）

**対象:** MainWindow の配線相当を UI なしで合成する結合テスト基盤 + 状態遷移/組み合わせシナリオ
**保証内容:** LTC フレーム列 × オペレーター操作列の任意の組み合わせで、判定→副作用の全ストリームが
仕様どおりであり、システム不変条件が破られない。
**基盤設計:**
- 新規 `tests/.../Integration/SyncScenarioHarness.cs`: 実物の `PlaylistState` / `TimecodeSyncService` /
  `SyncDecisionEngine` / `GapFreezeHandler` / `LtcSignalLossPolicy` / `ContinueOnTrackPlanner` 等と、
  3つの Coordinator（SingleModeSync / ContinueOnTrack / GapEnter）を **MainWindow と同じ配線**で組み、
  mpv 相当（time-pos/seek/pause/loadfile）だけを記録型フェイクにする。
  時刻は注入（単調ミリ秒）。「LTCフレームを1つ供給」「100ms tick」「モード切替」「Sync切替」
  「手動Play/Pause」「シークバー操作」をメソッドとして公開し、シナリオを DSL 的に書けるようにする
- ハーネス自身の自己検証テストを最初に書く（既知の単純シナリオが既存単体テストと同じ結論になること）
**シナリオ（状態遷移・組み合わせ）:**
1. **ショー進行フル**: Continue+Sync ON → トラック1追従 → ギャップ(Freeze) → トラック2切替 →
   信号断(Stop設定) → pause → 復帰 → 再同期 → 手動 Sync OFF → 手動シーク → Sync ON 復帰
2. **pause 所有権不変条件**: 信号断 pause / ギャップ pause / 手動 pause の3所有者について、
   任意の順序の組み合わせ（少なくとも全ペア×2順序=12系列）で「解除するのは所有者だけ」
   「手動操作は常に勝つ」が保たれる
3. **モード切替総当たり**: {Single, Continue} × {Sync ON, OFF} × {GapState 各値} ×
   {信号 受信中/断} の到達可能な組み合わせから各遷移を1ステップ実行し、不変条件
   （Inactive でない GapState は Continue+ON でのみ存在 / 二重 pause なし / 描画判定と状態の整合）を
   全遷移後に機械的にアサートする（モデルベース。到達不能な組み合わせはその根拠を記録）
4. **ギャップ境界の連打**: トラック末尾±1フレームを行き来する LTC 列で enter/exit が
   バタつかない（既存ヒステリシス/タイムアウトの合成検証）
5. **回帰シナリオ**: 今回ユーザーが発見した2バグ（スクロールバー起因ロード相当の「意図しない
   LoadCurrentPlaylistTrack」/ Black 固着）をシナリオとして再現し赤→現行実装で緑を確認
**対象ファイル:** `tests/.../Integration/`（新規ディレクトリ）、既存 src は変更禁止
（配線を再現できない場合は BLOCKED で報告 — 配線都合の本体リファクタは勝手にしない）
**実行コマンド:** 共通ルールの非E2E
**完了条件:** 上記5群が全て実配線で検証され、不変条件アサートが機械化されており、
ミューテーション確認（例: GapStateExitPolicy の条件反転でシナリオ3が赤）が記録されていること

## V9: E2E システムシナリオ（実アプリ・オペレーター操作列）

**対象:** FlaUI E2E に「複数機能を跨ぐ操作列」テストを追加（UIA パターン操作のみ。キー/マウス合成禁止）
**保証内容:** 実アプリ・実 mpv で、現場の代表的な一連の操作が通しで成立する。
**シナリオ:**
1. **プロジェクトラウンドトリップ**: プレイリスト構築（追加/並べ替え/オフセット編集）→ 保存 →
   クリア → 読込 → 全状態（トラック順・オフセット・モード・Gap設定）が一致 →
   その状態で再生・シークが動作
2. **モード切替ワークフロー**: 再生中に Single↔Continue を往復し、各切替後に再生位置・
   ボタン状態・タイムライン表示が矛盾しない
3. **フルスクリーン×再生の組み合わせ**: 再生中にフルスクリーン開閉を2周 → メイン表示・
   再生継続・Spout 状態が不変
4. **破損プロジェクト読込（V1連動）**: 破損ファイルを読込操作 → エラーが UI に出て
   アプリが継続使用できる（クラッシュ・フリーズしない）
**対象ファイル:** `tests/.../E2E/SystemScenarioE2ETests.cs`（新規、既存 E2E コレクション参加）
**実行コマンド:** 非E2E + E2E 全件（新規含む・Skip なし）
**完了条件:** 4シナリオ全て実アプリで緑、既存 E2E 全件緑、フレーキー対策
（WaitUntil ポーリング・固定 Sleep 禁止）が既存規約どおり

## V10: 実機 LTC 結合シナリオ（VB-CABLE・組み合わせ）

**対象:** `LtcHardwareLoopE2ETests` 基盤で、実信号での状態遷移組み合わせを追加
**保証内容:** 本物のオーディオ経路を通した信号で、信号断モード × Gap 動作 × モード切替の
組み合わせが仕様どおり動く。
**シナリオ:**
1. Continue + Gap=Black + 実 LTC がトラック範囲を出る → 黒 → Single へ切替 → **映像が即復帰**
   （今回の実バグの実機回帰テスト）
2. 信号断 Stop モード + ギャップ演出中に信号断 → ギャップが優先され信号断 pause が介入しない →
   ギャップ解除後に信号断評価が生きる（V6 の実機版・1ケース）
3. 実 LTC 供給中のモード往復（Single↔Continue 各2回）で追従が毎回回復する
**対象ファイル:** `tests/.../E2E/LtcHardwareLoopE2ETests.cs` または新規クラス（同コレクション・
同 Skip 規約）。信号は `LtcSignalPlayer` を使用
**実行コマンド:** 非E2E + E2E 全件（実機込み・Skip なし）
**完了条件:** 3シナリオ実機緑 + 実行時間の増分が計 5 分以内に収まる信号長設計

---

## 実行順（改訂）

**V1（データ破損）→ V8（結合ハーネス）→ V9（E2Eシナリオ）→ V10（実機組み合わせ）**
→ V5（オーバーフロー）→ V2 → V6 → V3 → V4 → V7。
結合・システム層（V8-V10）を単体系（V2-V7）より優先する。V6 の単体ケースのうち V8 の
シナリオ3で覆われるものは重複実装せず、V6 側は境界値（timeout 境界等）に縮小してよい。
V1 と V5 は実バグを暴く可能性があるため、発見時は該当タスクを止めて報告する
