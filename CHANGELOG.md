...
- **On material with a long keyframe interval, sync takes around 7 seconds to lock on after following starts** (measured on field material with a 6.6s interval, 16 runs under the same conditions: **6.7 to 8.2s, median 7.3s**). **This is not longer than before**: the published v0.4.3 measures **7.40 to 7.66s** under the same conditions. It is a long-standing property that this release does not remove; the mechanism that would shorten it broke other cases and was shelved (see Not shipped). A single seek leaves an error equal to the seek duration, so the landing is repeated until it converges. **A keyframe interval of 1 to 2 seconds brings this down to about 1.4 seconds.**
