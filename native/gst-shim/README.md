...
- **現在世代のフレームだけ**を返す。先頭が `generation` と一致しないフレームは破棄し、
  ...
  旧世代のデコード済みフレームは返らない。
- **滞留を有界にして返す**（問題 H/H-2 修正、2026-09-11）: 未配信 FIFO(4) を持ち、
  `acquire` は未配信数 n に応じて返す。n=0 は 0（なし）。n≤2 は最古を返す（1 tick に
  2 到着しても滞留は最大 1 枚）。n>2 は最古の n−2 枚を破棄してから残り 2 枚の古い方を返す
  （合成停止後は 1 tick で最新へ追い付く）。n=2 が 30 回連続したら最古を 1 枚破棄し、
  周期差による定常滞留を解消する。破棄は `TcsDeliveryStats.latest_replaced` と配信トレースの
  flags bit0 に記録する。判断は純関数 `tcs_delivery_plan`（`include/tcs_delivery_policy.h`）に
  分離し、shim 単体テストで固定する。
...
| 規則2 位置に基づく最新優先 | **部分一致** | position は受け取らず「現世代の未配信を、滞留 n≤2 は最古・n>2 は n−2 破棄して残り 2 枚の古い方」。位置選択は合成層（pts_ns 参照）。n=2 が 30 回連続したら 1 枚破棄 |
...
- **位置選択**: shim は position を受け取らず現世代の未配信を滞留有界（n≤2 は最古、
  n>2 は n−2 破棄）で 1 枚返すため、返却画像の `pts_ns` を
  `SourceImageStamp.PositionSeconds` として扱う。過去行列は持たない
  （問題 H-2 修正後は滞留最大 1 枚・停止後は 1 tick で最新へ追い付く）。
