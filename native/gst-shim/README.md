...
- **現在世代のフレームだけ**を返す。先頭が `generation` と一致しないフレームは破棄し、
  ...
  旧世代のデコード済みフレームは返らない。
- **滞留を有界にして返す**（問題 H/H-2/H-3 修正、2026-09-11）: 未配信 FIFO(4) を持ち、
  `acquire` は未配信数 n に応じて返す。n=0 は 0（なし）。n≤2 は最古を返す（1 tick に
  2 到着しても滞留は最大 1 枚）。n=2 で最古の到着から 21ms（60fps の 1.25 フレーム）以上
  経過したら最古を 1 枚破棄し、周期差による定常滞留を解消する（H-3。旧規則の「n=2 が
  30 回連続」を置換）。n>2 は最古の n−2 枚を破棄してから残り 2 枚の古い方を返す
  （合成停止後は 1 tick で最新へ追い付く）。破棄は `TcsDeliveryStats.latest_replaced` と
  配信トレースの flags bit0 に記録する。判断は純関数 `tcs_delivery_plan`
  （`include/tcs_delivery_policy.h`）に分離し、shim 単体テストで固定する。
...
- **位置選択**: shim は position を受け取らず現世代の未配信を滞留有界（n≤2 は最古、
  ...
  （問題 H-2/H-3 修正後は滞留最大 1 枚・定常は 21ms で 1 枚破棄・停止後は 1 tick で最新へ追い付く）。
