...
public sealed class ApplicationVersionTests
{
    ...
    {
        // 0.5.1への固定は、リリース成果物の意図しないバージョン変更を検出するリリースゲート。
        ApplicationVersion.Current.Should().Be("0.5.1");
        ApplicationVersion.WindowTitle.Should().Be("Timecode Sync Player v0.5.1");
    }
}
