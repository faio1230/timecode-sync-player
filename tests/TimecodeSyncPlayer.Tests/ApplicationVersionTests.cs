...
public sealed class ApplicationVersionTests
{
    ...
    {
        // 0.4.8への固定は、リリース成果物の意図しないバージョン変更を検出するリリースゲート。
        ApplicationVersion.Current.Should().Be("0.4.8");
        ApplicationVersion.WindowTitle.Should().Be("Timecode Sync Player v0.4.8");
    }
}
